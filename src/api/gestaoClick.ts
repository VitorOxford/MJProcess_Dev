...
type PaymentMethod = { id: number; nome: string; };
type PaymentCondition = { id: number; nome: string; };
...
  vendedor_id?: number;
  parcelas?: { parcela: { dias: number; porcentagem: string; forma_pagamento_id: number | null; } }[];
};
...
      if (!response.ok) throw new Error('Não foi possível buscar as formas de pagamento.');
      const data = await response.json();
      return data.data || [];
    } catch (error) { console.error('Erro em buscarFormasDePagamento:', error); throw error; }
  },

  async buscarCondicoesDePagamento(): Promise<PaymentCondition[]> {
    try {
      const response = await fetch(`${API_BASE_URL}/condicoes_pagamento`, { headers: getAuthHeaders() });
      if (!response.ok) throw new Error('Não foi possível buscar as condições de pagamento.');
      const data = await response.json();
      return data.data || [];
    } catch (error) { console.error('Erro em buscarCondicoesDePagamento:', error); throw error; }
  },
