...
type OrderItem = {
  ...
  unit_of_measure: 'metro' | 'kg' | null; // Adicionado
  quantity_unit: number | null; // Adicionado
};
  ...
  if (!parentOrder) {
   ...
   try {
    ...
    if (item.unit_of_measure === 'kg') {
        ...
        const yieldInMeters = item.quantity_meters.toLocaleString('pt-BR', { maximumFractionDigits: 2 });
        quantityDisplay = `${originalKg}kg (Rendimento: ~${yieldInMeters}m)`;
    }
    // --- FIM DA CORREÇÃO ---
    ...
    autoTable(doc, {
        ...
        body: [[
            item.fabric_type,
            item.stamp_ref,
            quantityDisplay // <-- Variável corrigida aqui
        ]],
        theme: 'grid',
        headStyles: { fillColor: [41, 128, 185] }
    });
