                  ...
                  <v-card class="item-list-card" variant="outlined">
                    <v-list class="bg-transparent">
                      ...
                      >
                        ...
                        <v-list-item-subtitle>{{ item.fabric_type || 'Sem tecido' }} - {{ item.quantity_meters || 0 }}{{item.unit_of_measure}}</v-list-item-subtitle>
                        <template v-slot:append>
                        ...
                        <v-row>
                          ...
                          <v-col cols="12" sm="6">
                            <v-autocomplete
                              ...
                              v-model:search="stampSearch"
                              :items="filteredGestaoClickServices"
                              item-title="nome"
                              item-value="id"
                              ...
                              placeholder="Digite para buscar a estampa..."
                              no-filter
                            >
                               <template v-slot:item="{ props, item }">
                              ...
                              <template v-slot:no-data>
                                 <v-list-item title="Digite ao menos 2 caracteres para buscar."></v-list-item>
                              </template>
                            </v-autocomplete>
                             <v-text-field
                                v-else
...
const rules = {
  ...
  fileSize: (v: File[] | null) => !v || v.length === 0 || v[0].size < 5000000 || 'O arquivo não pode exceder 5 MB!',
};
...
const filteredGestaoClickServices = computed(() => {
    if (!stampSearch.value || stampSearch.value.length < 2) {
        return [];
    }
    const query = stampSearch.value.toLowerCase();
    return gestaoClickServices.value.filter(service =>
...
const syncOrderWithGestaoClick = async () => {
    ...
    const salePayload: any = {
        ...
        parcelas: paymentDetails.installments.map(inst => {
            const startDate = new Date(new Date().setHours(0,0,0,0));
            const dueDate = new Date(new Date(inst.due_date).setUTCHours(12,0,0,0));
            const diffTime = dueDate.getTime() - startDate.getTime();
            const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
            return { parcela: {
                dias: diffDays >= 0 ? diffDays : 0,
                porcentagem: ((inst.value / totalOrderValue.value) * 100).toFixed(2),
